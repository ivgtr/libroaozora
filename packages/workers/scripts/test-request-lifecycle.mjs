...
const core = versioned ? await import('../../core/dist/index.js') : null
const revision = versioned ? await core.sourceRevision(fixture.metadata) : null
const snapshot = versioned ? JSON.stringify({ ...JSON.parse(metadata), schemaVersion: 1, generation: 'http-test' }) : null
const entry = versioned ? {
  schemaVersion: 2, workId: fixture.metadata.id, sourceRevision: revision,
  sourceUrl: fixture.metadata.sourceUrls.text, fetchedAt: '2026-09-06T00:00:00Z',
  zipHash: await core.sha256(zip), textHash: await core.sha256(fixture.text),
  decodeVersion: core.DECODE_VERSION, contentId: core.contentIdentifier(await core.sha256(fixture.text)), text: fixture.text,
} : null
const rows = []
function socket(url) {
  let req
...
try {
  for (const timing of ['after-disconnect', 'before-disconnect']) for (const phase of ['metadata-kv', 'metadata-r2', 'metadata-body', 'content-kv', 'content-r2', 'content-body', 'origin', 'origin-body', 'origin-timeout']) {
    ...
    try {
      ...
      if (versioned) {
        await r2.put('metadata/current.json', JSON.stringify({ schemaVersion: 1, current: { generation: 'http-test', digest: await core.sha256(snapshot) }, previous: null }))
        await r2.put('metadata/snapshots/http-test.json', snapshot)
        if (savedZip) await r2.put(`content/v2/047927/${revision}.zip`, zip, { customMetadata: Object.fromEntries(['sourceUrl', 'fetchedAt', 'sourceRevision', 'zipHash', 'textHash', 'decodeVersion'].map(key => [key, entry[key]])) })
      }
      ...
      await until(async () => (await (await fetch(base + '/__status')).json()).retained >= before.retained + (phase.startsWith('metadata') ? 1 : 2), `${phase}: follower did not retain shared work`)
      ...
      const expected = phase === 'origin-timeout' ? (versioned ? 503 : 500) : 200
      assert.equal(result.status, expected, result.text)
      if (phase === 'origin-timeout') {
        assert.deepEqual(JSON.parse(result.text), { error: versioned ? { code: 'SOURCE_TEMPORARY_ERROR', message: 'Content source unavailable' } : { code: 'INTERNAL_ERROR', message: 'Internal server error' } })
        release()
        await kv.put(versioned ? `content:v2:047927:${revision}` : 'content:047927', versioned ? JSON.stringify(entry) : fixture.text)
      }
  ...
  console.log(JSON.stringify({ runtime: 'local workerd over separate HTTP sockets', versioned, retentionEnabled: !process.argv.includes('--without-retention'), rows }, null, 2))
