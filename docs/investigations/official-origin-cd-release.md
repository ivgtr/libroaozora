...
移行完了の受入れでは、legacyを読み込んだwarm readerを使い、60秒を待たずに初回writer公開直後の本文要求が成功することも確認する。マーカーを観測したreaderは同じ要求内でcurrentを一度再取得するため、キャッシュ済みの不存在だけを理由とする一過性503を発生させない。実際のpointer欠落・破損・通信失敗は503のまま。
