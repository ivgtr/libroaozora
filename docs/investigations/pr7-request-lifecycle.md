...
PR #8の既存コミットe2c3b7c（metadata復旧・移行完了後のpointer再確認を含む）を保持して、先行PRをmergeした。versioned本文、共有旧版候補、metadata要求、世代別snapshot読込みへcontextを伝達する。metadata要求は20秒、snapshot読込みは5秒で回収する。期限切れのmetadata処理が後着しても新しいpointer/last状態を上書きしないよう、候補状態を分け、現タスクであることとabort状態を確認して反映する。移行済みの認識は単調に維持する。

core61 / Workers141 / Node同期8 tests、workspace lint・scripts/testsの型、core build、Wrangler dry-run成功。世代別読込みのretention・期限回収を追加で検証した。新APIでも同じ実HTTP切断18シナリオを実施し、正常200/期限切れ503、その後の再アクセスと枠回復200を確認した。[PR #8の結果](./pr8-request-lifecycle.json)。旧版共有のキーにprevious generation/digestを含める既存修正を維持した。本番デプロイ・同期・CDN操作は未実施。
