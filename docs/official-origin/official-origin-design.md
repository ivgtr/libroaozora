...
移行マーカーとキャッシュ済みのcurrent不存在が食い違う場合は、その共有要求内でcurrentを1回だけ再取得する。正常pointerならsnapshot取得へ進み、checkedAtもその再確認時刻に更新する。再取得が欠落・破損・通信失敗なら503を維持し、legacyへは戻らない。これにより、60秒以内に初回公開が完了した場合の最初の要求群も正常応答できる。
